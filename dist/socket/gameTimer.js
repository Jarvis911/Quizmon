 ...
 */
export async function startQuestionTimer(io, matchId) {
 ...
 */
export async function skipToNextQuestion(io, matchId) {
    const matchState = await getMatch(matchId);
    if (!matchState)
        return;
 ...
 */
export async function processTimeUp(io, matchId) {
    const matchState = await getMatch(matchId);
    if (!matchState)
        return;
    // Clear timer if still running
    ...
    await saveMatch(matchId, matchState);
    // Emit updated scores
    io.to(String(matchId)).emit('updatedScores', matchState.players.map((p) => ({
        userId: p.userId,
        username: p.username,
        score: p.score,
    })));
