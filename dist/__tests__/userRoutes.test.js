...
describe('User Routes', () => {
    ...
    describe('GET /user/statistics', () => {
        it('should get user statistics successfully', async () => {
            ...
            prismaMock.matchResult.count.mockResolvedValueOnce(1);
            prismaMock.matchResult.findMany
                .mockResolvedValueOnce(mockResults) // paged results
                .mockResolvedValueOnce(mockAllResults); // all results in those matches (for rank)
        ...
        it('should return empty statistics if no results', async () => {
            prismaMock.matchResult.count.mockResolvedValueOnce(0);
            prismaMock.matchResult.findMany.mockResolvedValueOnce([]);
            const response = await request(app).get('/user/statistics');
            expect(response.status).toBe(200);
