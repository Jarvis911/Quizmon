...
export const createJob = async (req, res) => {
    try {
        ...
        try {
            const generationResult = await generateQuestions(instruction || null, pdfText, imageParts.length > 0 ? imageParts : null, count, types);
            ...
            await notificationService.createNotification(Number(userId), `AI đã tạo xong câu hỏi cho bạn. Hãy vào kiểm tra nhé!`, 'AI_GENERATION_COMPLETED', `/ai-generation/jobs/${job.id}`);
            ...
            await notificationService.createNotification(Number(userId), `Tạo câu hỏi bằng AI thất bại: ${aiError.message}`, 'AI_GENERATION_FAILED', `/ai-generation/jobs/${job.id}`);
            res.status(500).json({
...
export const approveAllAndCreateQuiz = async (req, res) => {
    try {
        ...
        // Send notification
        await notificationService.createNotification(Number(userId), `Bộ câu hỏi AI "${quiz.title}" đã được thêm vào thư viện của bạn.`, 'AI_QUIZ_APPROVED', `/library/${quiz.id}`);
...
export const getAgentChatSessions = async (req, res) => {
    try {
        const userId = req.userId;
...
export const getAgentChatSession = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
...
// Delete chat session
export const deleteAgentChatSession = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
...
// Rename chat session
export const renameAgentChatSession = async (req, res) => {
    try {
        const { id } = req.params;
