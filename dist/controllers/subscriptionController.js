 ...
 */
export const checkoutFree = async (req, res) => {
    try {
        const { planId, promotionId, billingCycle } = req.body;
        const organizationId = req.organizationId;
        if (!organizationId) {
            res.status(400).json({ message: 'Organization context required' });
            return;
        ...
        const subscription = await fulfillSubscription(orderId, organizationId, Number(planId), cycle, PaymentMethod.MOCK);
        res.status(200).json(subscription);
    }
    catch (err) {
