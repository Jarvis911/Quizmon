...
export const createHomeworkMatch = async (req, res) => {
    try {
        ...
        if (students.length > 0) {
            ...
            const emailPromises = students
                ...
                .map(student => emailService.sendEmail(student.user.email, "Bài tập mới trên Quizmon!", `<h2>Chào ${student.user.username || 'bạn'},</h2>
...
export const submitHomeworkAnswer = async (req, res) => {
    try {
        const { id } = req.params; // matchId
        ...
        const validationResult = checkAnswer(question, answerData);
...
// Finish homework
export const finishHomework = async (req, res) => {
    try {
        const { id } = req.params; // matchId
        const userId = req.userId;
        if (!userId) {
