...
export const createClassroom = async (req, res) => {
    try {
        ...
        await notificationService.createNotification(Number(userId), `Bạn đã tạo thành công lớp học: ${classroom.name}`, 'CLASSROOM_CREATED', `/classrooms/${classroom.joinCode}`);
...
export const joinClassroom = async (req, res) => {
    try {
        ...
        await notificationService.createNotification(Number(userId), `Yêu cầu tham gia lớp "${classroom.name}" đang chờ giáo viên duyệt.`, 'CLASSROOM_PENDING', `/classrooms/${classroom.joinCode}`);
        await notificationService.createNotification(classroom.teacherId, `Học sinh ${member.user.username || 'mới'} xin tham gia lớp: ${classroom.name}`, 'STUDENT_JOIN_REQUEST', `/classrooms/${classroom.joinCode}`);
...
export const joinByInviteLink = async (req, res) => {
    try {
        ...
        await notificationService.createNotification(classroom.teacherId, `Học sinh ${member.user.username || 'mới'} xin tham gia lớp: ${classroom.name} (qua link mời)`, 'STUDENT_JOIN_REQUEST', `/classrooms/${classroom.joinCode}`);
        res.status(201).json({
...
export const approveMember = async (req, res) => {
    try {
        ...
        await notificationService.createNotification(member.user.id, `Yêu cầu tham gia lớp "${classroom.name}" đã được duyệt!`, 'CLASSROOM_APPROVED', `/classrooms/${classroom.joinCode}`);
...
export const rejectMember = async (req, res) => {
    try {
        ...
        await notificationService.createNotification(member.user.id, `Yêu cầu tham gia lớp "${classroom.name}" đã bị từ chối.`, 'CLASSROOM_REJECTED', '');
...
export const matchExpectedStudent = async (req, res) => {
    try {
        const id = String(req.params.id);
...
export const unmatchExpectedStudent = async (req, res) => {
    try {
        const id = String(req.params.id);
        const expectedId = String(req.params.expectedId);
        const userId = req.userId;
...
export const importExpectedStudents = async (req, res) => {
    try {
        ...
        // ── Persist to DB ─────────────────────────────────────────────────
        const created = await Promise.all(students.map(s => prisma.expectedStudent.create({
            data: {
                name: s.name,
...
export const clearExpectedStudents = async (req, res) => {
    try {
        const id = String(req.params.id);
        const userId = req.userId;
        const classroom = await prisma.classroom.findUnique({ where: { id: parseInt(id) } });
