...
export const createQuestion = async (questionData) => {
    try {
        const { quizId, text, type, media = [], options, correctAnswer, correctLatitude, correctLongitude, radius1000, radius750, radius500, mapType, } = questionData;
...
export const updateQuestion = async (id, questionData) => {
    try {
        const { text, type, media = [], options = [], correctAnswer, correctLatitude, correctLongitude, radius1000, radius750, radius500, mapType, } = questionData;
        const dataJson = type
        ...
        // Send notification
        if (updated.quiz) {
            await notificationService.createNotification(updated.quiz.creatorId, `Bạn đã cập nhật câu hỏi trong bộ: ${updated.quiz.title}`, 'QUESTION_UPDATED', `/library/${updated.quiz.id}`);
        }
        return updated;
    }
    catch (err) {
