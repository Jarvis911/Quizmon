 ...
 */
export async function hydrateAiQuizVisuals(jobId, generation) {
    ...
    try {
        ...
        if (coverImg) {
            ...
            const url = await uploadBufferToAzure(coverImg.buffer, `ai-quiz-${jobId}-cover.${ext}`, coverImg.mimeType);
    ...
    for (let i = 0; i < rows.length && i < generation.questions.length; i++) {
        ...
        try {
            ...
            if (img) {
                ...
                const url = await uploadBufferToAzure(img.buffer, `ai-quiz-${jobId}-q-${row.id}.${ext}`, img.mimeType);
...
export async function applyVisualsToRegeneratedRow(generatedQuestionId, newQuestion) {
    if (process.env.AI_QUIZ_GENERATE_IMAGES === 'false') {
        return;
    }
    ...
    try {
        ...
        const ext = img.mimeType.includes('png') ? 'png' : 'jpeg';
        const url = await uploadBufferToAzure(img.buffer, `ai-quiz-regen-q-${generatedQuestionId}.${ext}`, img.mimeType);
        await prisma.aIGeneratedQuestion.update({
            where: { id: generatedQuestionId },
            data: {
                generatedImageUrl: url,
