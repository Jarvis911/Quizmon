 ...
 */
export const createOrganization = async (name: string, userId: number) => {
...
export const updateOrganization = async (
    ...
    const updateData: { name?: string; logoUrl?: string; slug?: string } = { ...data };
    if (data.name) {
 ...
 */
export const getMemberRole = async (orgId: number, userId: number) => {
    const member = await prisma.organizationMember.findUnique({
        where: { organizationId_userId: { organizationId: orgId, userId } },
    });
